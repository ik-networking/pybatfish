...
__all__ = ["format_header_space_explanation",
           "format_search_filters_frame"]
...
class AclIpSpace(namedtuple("AclIpSpace", "lines")):
    ...
    def simplify(self):
        ...
        if (len(lines) == 3 and
            ...
            ipSpace0 = lines[0].ipSpace
            ipSpace1 = lines[1].ipSpace
...
class IpSpaceReference(namedtuple("IpSpaceReference", "name description")):
    __slots__ = ()
...
class MatchSrcInterface(namedtuple("MatchSrcInterface", "srcInterfaces")):
    __slots__ = ()
...
def parseIpSpaceReference(obj):
    return IpSpaceReference(obj['name'], obj['description'])


aclLineMatchExprParsers["org.batfish.datamodel.IpSpaceReference"] = parseIpSpaceReference
...
def format_header_space_explanation(jsonStr):
    obj = loads(jsonStr)
    ast = parse(obj)
...
def format_search_filters_frame(frame):
    frame['Header_Space'] = frame['Header_Space'].apply(
        format_header_space_explanation)
    return frame
